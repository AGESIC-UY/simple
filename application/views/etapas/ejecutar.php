...
<ul class="step-boxes<?= (count($etapa->getPasosEjecutables()) == 1 ? ' hidden' :'')?>">
    <?php
        ...
        for($i = 1; $i <= count($etapa->getPasosEjecutables()); $i++) {
            ...
            ?>
            <li id="step_<?=$i?>" <?= ($i == $current_position ? ' tabindex="-1"' : '')?> class="step-box<?= ($i == $current_position ? ' active' : '')?>" style="width:<?=number_format((100/count($etapa->getPasosEjecutables())), 4, '.', '') ?>%;">
...
<script>
     $(document).ready(function () {
         <?php if ($step_position>1): ?>
         $("#step_<?=($step_position)?>").focus();
         <?php endif; ?>
     });
</script>