...
class UsuarioBackendSesion {
    ...
    public static function force_login() {
        ...
        if (!$u) {
            ...
            //si cambio de cuenta tengo que mudar al usuario
    ...
    public static function login($email, $password, $cuenta_id) {
        $CI = & get_instance();

        $autorizacion = self::validar_acceso($email, $password, $cuenta_id);

        if ($autorizacion) {
            $u = Doctrine::getTable('UsuarioBackend')->findOneByEmailAndCuentaId($email, $cuenta_id);
    ...
    public static function validar_acceso($email, $password, $cuenta_id = false) {
        if (!$cuenta_id) {
    ...
    public static function validar_acceso_saml($usuario, $cuenta_id) {
        $usuario = Doctrine::getTable('UsuarioBackend')->findOneByUsuarioAndCuentaId($usuario, $cuenta_id);
    ...
    public static function login_ldap($usuario, $password, $cuenta_id) {
        ...
        if ($bind = ldap_bind($ds, $ldapconfig['user_con'], $ldapconfig['pass_con'])) {
            ...
            if ($count == 1) {
                if ($bind = ldap_bind($ds, $userdn, $password)) {
                    $CI = & get_instance();
                    $u = self::validar_acceso_ldap($usuario, $cuenta_id);
    ...
    public static function has_rol($rol) {
        $usuario = self::$user;
    ...
    public static function isAuditor() {
        $usuario = self::$user;
        return $usuario->seg_alc_auditor;
    }
