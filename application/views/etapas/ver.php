<script>
    $(document).ready(function () {
        $("#btn_siguiente").click(function (event) {
            ...
            if ("<?= $sidebar ?>" === "firma_por_lotes") {
                window.location.href = "<?= site_url('etapas/ver_firma/' . $etapa->id . '/' . ($secuencia + 1)) ?>";
            } else {
...
<input type="hidden" id="info_tramite_nro_paso" value="<?php
if ($step_position == 0) {
    echo 'paso_inicio';
} else {
    echo 'paso_' . ($step_position + 1);
}
?>" />
...
<?php endif; ?>
        ...
        <li class="action-buttons-second">
            <ul>
                <li class="float-left">
<?php if ($secuencia > 0 && $sidebar == 'firma_por_lotes'): ?><a class="btn btn-link btn-lg"  href="<?= site_url('etapas/ver_firma/' . $etapa->id . '/' . ($secuencia - 1)) ?>" ><span class="icon-chevron-left"></span> Volver</a><?php elseif ($secuencia > 0): ?>
                        <a class="btn btn-link btn-lg"  href="<?= site_url('etapas/ver/' . $etapa->id . '/' . ($secuencia - 1)) ?>" ><span class="icon-chevron-left"></span> Volver</a><?php endif; ?>
                </li>
            </ul>
        </li>
    </ul>
