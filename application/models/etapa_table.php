...
class EtapaTable extends Doctrine_Table {
    ...
    public function cantidadSinAsignar($usuario_id, $cuenta = 'localhost') {
        ...
        $stmt = $conn->prepare('select COUNT(distinct id) from (
            ...
            where e.usuario_id is NULL and c.id = ' . $cuenta->id . '
            ...
            where e.usuario_id is NULL  and c.id = ' . $cuenta->id . ' ' . $opciones_busqueda . '
    ...
    public function findSinAsignarConPaginacion($usuario_id, $cuenta = 'localhost', $orderby, $direction, $per_page, $offset) {
        set_time_limit(1600);
        ini_set('memory_limit', '-1');
    ...
    public function findSinAsignarFiltro($usuario_id, $cuenta = 'localhost', $orderby, $direction, $busqueda_id_tramite, $busqueda_etapa, $busqueda_grupo, $busqueda_nombre, $busqueda_documento, $busqueda_modificacion_desde, $busqueda_modificacion_hasta, $per_page, $offset, $count) {
        set_time_limit(1600);
        ini_set('memory_limit', '-1');
        ...
        if ($busqueda_modificacion_desde) {
            //para que el strtotime la tome como d/m/y
            $busqueda_modificacion_desde = str_replace('/', '-', $busqueda_modificacion_desde);
        ...
        if ($busqueda_modificacion_hasta) {
            //para que el strtotime la tome como d/m/y
            $busqueda_modificacion_hasta = str_replace('/', '-', $busqueda_modificacion_hasta);
    ...
    public function findPendientes($usuario_id, $cuenta = 'localhost', $orderby = 'updated_at', $direction = 'desc') {
        if ($orderby == "proceso_nombre") {
            $orderby = "p.nombre";
        } else if ($orderby == "tarea_nombre") {
            $orderby = "tar.nombre";
        }
        $query = Doctrine_Query::create()
    ...
    public function findPendientesCiudadanoFuncionarioConPaginacion($usuario_id, $cuenta = 'localhost', $orderby = 'updated_at', $direction = 'desc', $per_page, $offset) {
        if ($orderby == "proceso_nombre") {
            $orderby = "p.nombre";
        } else if ($orderby == "tarea_nombre") {
            $orderby = "tar.nombre";
        ...
        $query = Doctrine_Query::create()
                ...
                //Si la etapa se encuentra pendiente y asignada al usuario
                ->where('e.pendiente = 1 and u.id = ?', $usuario_id)
                //la tarea tiene que ser la inicial
                ->andWhere('tar.inicial = 1')
                //la tarea no tiene que tener asignacion por grupo porque es un funcionario y no se quiere que otro funcionario tome las tareas de este otro (problema de seguridad)
                ->andWhere('tar.acceso_modo != "grupos_usuarios"')
                //Si la tarea se encuentra activa
                ->andWhere('1!=(tar.activacion="no" OR ( tar.activacion="entre_fechas" AND ((tar.activacion_inicio IS NOT NULL AND tar.activacion_inicio>NOW()) OR (tar.activacion_fin IS NOT NULL AND NOW()>tar.activacion_fin) )))')
                ->orderBy($orderby . ' ' . $direction)
                ->limit($per_page)
    ...
    public function findPendientesFiltro($usuario_id, $cuenta = 'localhost', $orderby = 'updated_at', $direction = 'desc', $busqueda_id_tramite, $busqueda_etapa, $busqueda_grupo, $busqueda_nombre, $busqueda_documento, $busqueda_modificacion_desde, $busqueda_modificacion_hasta, $per_page, $offset) {
        if ($orderby == "proceso_nombre") {
            $orderby = "p.nombre";
        } else if ($orderby == "tarea_nombre") {
            $orderby = "tar.nombre";
        ...
        if ($busqueda_modificacion_desde) {
            //para que el strtotime lo tome como d-m-y
            $busqueda_modificacion_desde = str_replace('/', '-', $busqueda_modificacion_desde);
            $busqueda_modificacion_desde = $busqueda_modificacion_desde . ' 00:00:00';
    ...
    public function findEtapasFirmaPorLote($id_usuario, $proceso_id, $contar, $per_page = null, $offset = null,$busqueda_id_tramite= null, $busqueda_id_etapa= null, $busqueda_nombre_tarea= null, $busqueda_modificacion_desde= null, $busqueda_modificacion_hasta= null) {
        if (!isset($offset) || !$offset) {
            $offset = 0;
        }
        $query_filtro="";        
        if ($busqueda_nombre_tarea!="" && $busqueda_nombre_tarea!=null) {
            $query_filtro.=' and t.nombre like "%' . $busqueda_nombre_tarea . '%" ';
        }
        if ($busqueda_id_etapa!="" && $busqueda_id_etapa!=null) {
            $query_filtro.=' and e.id like "%' . $busqueda_id_etapa . '%" ';
        }
        if ($busqueda_id_tramite!="" && $busqueda_id_tramite!=null) {
            $query_filtro.=' and e.tramite_id like "%' . $busqueda_id_tramite . '%" ';
        }
        if ($busqueda_modificacion_desde!="" && $busqueda_modificacion_desde!=null) {
            //para que el strtotime lo tome como d-m-y
            $busqueda_modificacion_desde = str_replace('/', '-', $busqueda_modificacion_desde);
            $busqueda_modificacion_desde = $busqueda_modificacion_desde . ' 00:00:00';
            $query_filtro.=' and e.updated_at >= "'. date("Y-m-d H:i:s", strtotime($busqueda_modificacion_desde)).'"';
        }
        if ($busqueda_modificacion_hasta!="" && $busqueda_modificacion_hasta!=null) {
            //para que el strtotime lo tome como d-m-y
            $busqueda_modificacion_hasta = str_replace('/', '-', $busqueda_modificacion_hasta);
            $busqueda_modificacion_hasta = $busqueda_modificacion_hasta . ' 00:00:00';
            $query_filtro.=' and e.updated_at <= "'. date("Y-m-d H:i:s", strtotime($busqueda_modificacion_hasta)).'"';
        }  
        $query = '
            (select distinct * from
            (
              select e.* from
              etapa e join tarea t on t.id=e.tarea_id
              join proceso p on p.id=t.proceso_id
              join cuenta c on c.id = p.cuenta_id
              join usuario u on u.id = ' . $id_usuario . '
              join grupo_usuarios_has_usuario gu on gu.usuario_id=u.id
              join formulario f on p.id = f.proceso_id
              JOIN paso pa ON pa.tarea_id = t.id  AND pa.formulario_id = f.id
              join campo x on x.formulario_id = f.id
              where e.usuario_id is NULL and c.id = u.cuenta_id
              and t.proceso_id = ' . $proceso_id . '
              '.$query_filtro.' 
              and x.tipo = "documento"
              and (JSON_EXTRACT_C(x.extra, "$.firmar") = "on"
              or JSON_EXTRACT_C(x.extra, "$.firmar_servidor") = "on")
              and t.acceso_modo="grupos_usuarios"
              and FIND_IN_SET(gu.grupo_usuarios_id , t.grupos_usuarios)
              union all
              ...
              join campo x on x.formulario_id = f.id
              join grupo_usuarios_has_usuario gu on gu.usuario_id=u.id
              join etapa e2 on e.tramite_id = e2.tramite_id
              and e2.id < e.id and e2.pendiente = 0
              where e.usuario_id is NULL and c.id =  u.cuenta_id
              and t.proceso_id = ' . $proceso_id . '
              '.$query_filtro.' 
              and x.tipo = "documento"
              and (JSON_EXTRACT_C(x.extra, "$.firmar") = "on"
              or JSON_EXTRACT_C(x.extra, "$.firmar_servidor") = "on")
              and gu.grupo_usuarios_id =
              (select replace(valor, "\"","")
              from dato_seguimiento d
              where d.nombre = replace(t.grupos_usuarios,"@@","")
              and d.etapa_id = e2.id)
              union(
                select e.* from etapa e
                join tarea t on t.id=e.tarea_id
                ...
                join usuario u on u.id = ' . $id_usuario . '
                where e.usuario_id is NULL
                and c.id = u.cuenta_id
                    and x.tipo = "documento"
                    and (JSON_EXTRACT_C(x.extra, "$.firmar") = "on"
                    or JSON_EXTRACT_C(x.extra, "$.firmar_servidor") = "on")
                    and t.proceso_id = ' . $proceso_id . '
                    '.$query_filtro.' 
                    and ( (t.acceso_modo="publico") or (t.acceso_modo="registrados" and u.registrado=1)
                    or (t.acceso_modo = "claveunica" AND u.open_id=1) ))) as a
                    )
                    UNION
                    (SELECT e.*
                    FROM etapa e
                    ...
                    ';
        if (!$contar) {
            $query .= ' order by updated_at desc limit ' . $offset . ',' . $per_page . ';';
        } else {
            $query .= ';';
            $q = Doctrine_Manager::getInstance()->getCurrentConnection();
            return $q->execute($query)->fetchAll();
        }
        $q = Doctrine_Manager::getInstance()->getCurrentConnection();
        return $q->execute($query)->fetchAll();
    }

    public function findDocumentosFirmaPorLote($id_etapas) {
        $q = $id_etapas;
        $query = '
          select e.id etapa, e.tramite_id tramite, e.tarea_id tarea, x.id id, x.documento_id documento from etapa e
            join tarea t on t.id=e.tarea_id
            join formulario f on t.proceso_id = f.proceso_id
            join paso p on p.formulario_id = f.id and p.tarea_id = t.id
            join campo x on x.formulario_id = f.id
            where
              e.id in(' . implode(",", $id_etapas) . ')
              and x.tipo = "documento"
              and (JSON_EXTRACT_C(x.extra, "$.firmar") = "on"
              or JSON_EXTRACT_C(x.extra, "$.firmar_servidor") = "on");
            ';
