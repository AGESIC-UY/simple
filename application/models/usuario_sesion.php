...
class UsuarioSesion {
    ...
    public static function force_login() {
        ...
        $CI->load->helper('auditoria_helper');
    ...
    public static function login($usuario, $password) {
        ...
        if ($u) {
            ...
            foreach ($u->GruposUsuarios as $grupo) {
                ...
                if ($grupo->nombre == 'UsuarioFirmaPorLotes') {
                    $esUsuarioFirmaPorLotes = true;
                    continue;
                }
            }

            $CI->session->set_userdata('usuarioMesaDeEntrada', $esUsuarioMesaDeEntrada);
            $CI->session->set_userdata('usuarioFirmaPorLotes', $esUsuarioFirmaPorLotes);
    ...
    public static function registrado_saml() {
    ...
    public static function registrado_ldap() {
        if (self::usuario()) {
            return true;
        } else {
            return false;
    ...
    public static function limpiar_sesion() {
        $CI = & get_instance();
    ...
    public static function registrar_acceso($cuenta_id = false) {
        $CI = & get_instance();
    ...
    public static function login_o_registrar_empresa($rut, $razon_social, $email, $cuenta_id) {
        ...
        if ($usuario_empresa_encontrada) {
            ...
            $usuario->save();
            auditar('Usuario', "insert", $usuario->id, $usuario->usuario);
    ...
    public static function borrar_todos_los_datos_sesion() {
        ...
        foreach ($user_data_session as $key => $value) {
            if ($key != 'usuario_nivel_confianza' && $key != 'session_id' && $key != 'ip_address' && $key != 'user_agent' && $key != 'last_activity'&& $key != 'usuario_backend_id') {
    ...
    public static function usuarioFirmaPorLotes() {
        $CI = & get_instance();
        if (!$CI->session->userdata('usuarioFirmaPorLotes')) {
            return false;
        } else {
            return true;
        }
