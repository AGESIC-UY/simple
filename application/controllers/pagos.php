...
class Pagos extends MY_Controller {
        ...
        if (!$no_enviar) {
            ...
            if (($destinatario) && (strlen($message) > 0)) {
                ...
                /* $CI->load->helper('enviar_email');
                  enviar_emails($from, $cuenta->nombre_largo, $destinatario, $subject, $message); */
            }
        }
    }
    ...
    public function consulta_estado() {
        if ($this->input->post('IdSol') && $this->input->post('IdTramite')) {
            ...
            $pasarela = Doctrine::getTable('PasarelaPagoAntel')->find($pasarela_id);
            ...
            $existe_pago = Doctrine_Query::create()
                    ...
                    ->orderBy('p.id', 'DESC')
                    ->fetchOne();
            $existe_pago_primero = Doctrine_Query::create()
                    ->from('Pago p')
                    ->where('p.id_solicitud = ?', $id_solicitud)
                    ->orderBy('p.id', 'ASC')
                    ->fetchOne();
            ...
            if (!empty($existe_pago)) {
                switch ($existe_pago->estado) {
                    ...
                    case 'pendiente' || 'iniciado' || 'token_solicita':
                        $clave_tramite = Doctrine::getTable('DatoSeguimiento')->findOneByNombreAndEtapaId(md5($existe_pago_primero->id . '_clave_tramite_pasarela_pagos'), $existe_pago_primero->id_etapa);
                        ...
                        if ($curl_errno > 0 || $http_code != 200) {
                            ...
                            switch ($nuevo_estado[0]) {
                                ...
                                case '1':
                                    $registro_pago_old = Doctrine_Query::create()
                                            ->from('Pago p')
                                            ->where('p.id_solicitud = ?', $id_solicitud)
                                            ->orderBy('p.id', 'DESC')
                                            ->fetchOne();
                                    if (!empty($registro_pago_old)) {
                                        $registro_pago = new Pago();
                                        $registro_pago->id_solicitud = $id_solicitud;
                                        $registro_pago->id_tramite_interno = $registro_pago_old->id_tramite_interno;
                                        $registro_pago->id_tramite = $registro_pago_old->id_tramite;
                                        $registro_pago->id_etapa = $registro_pago_old->id_etapa;
                                        $registro_pago->pasarela = $registro_pago_old->pasarela;
                ...
                $etapa = Doctrine::getTable('Etapa')->find($existe_pago->id_etapa);

                preg_match('/(' . $etapa->id . ')\/([0-9]*)/', $_SERVER['HTTP_REFERER'], $match);
                if (!$match) {
                    $secuencia = 0;
                } else {
                ...
                if ($pasarela) {
                    ...
                    if ($dato) {
                        if ($dato->valor == 'pendiente') {
                            //show_error($pasarela->descripcion_pendiente_traza);
                            $regla = new Regla($pasarela->descripcion_pendiente_traza);
                            $descripcion_traza = $regla->getExpresionParaOutput($etapa_id);
                ...
                enviar_traza_linea_pago($etapa, $secuencia, $descripcion_traza, $pago_realizado);
            } else {
                $estado = [];
                $estado['estado'] = 'error';
                $estado['titulo'] = 'Pago no realizado';
        ...
        if ($this->input->post('IdSol')) {
         ...
         <soap:Body>
            ...
            if ($curl_errno > 0 || $http_code != 200) {
                ...
                $dato->valor = (string) isset($pago_fecha[0])?$pago_fecha[0]: date("d-mm-YY");
                $dato->etapa_id = $etapa_id;
                $dato->save();
                ...
                $dato->valor = (string) (count($pago_concilia)>0 ? $pago_concilia[0]: date("d-mm-YY H:m:s"));
                $dato->etapa_id = $etapa_id;
                $dato->save();
                ...
                $dato->valor = (string) (count($pago_rechazo)>0?$pago_rechazo[0]:0);
                $dato->etapa_id = $etapa_id;
                $dato->save();
                ...
                $dato->valor = (string) (count($pago_des_error)>0?$pago_des_error[0]:0);
                $dato->etapa_id = $etapa_id;
                $dato->save();
                ...
                switch ($nuevo_estado[0]) {
                    ...
                    case '9':
                        ...
                        if ($registro_pago_old != 'realizado') {
