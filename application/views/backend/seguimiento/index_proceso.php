...
<?php if(count($procesos_arch)>1){?>
<form id="procArchivadoForm" method="POST" action="backend/seguimiento/index_proceso/" class="form-inline">
    <label for="proc_arch_id_seg">Versiones anteriores</label>
    <select id="proc_arch_id_seg" name="proc_arch_id_seg" class="AlignText input-small">
        <?php foreach($procesos_arch as $proceso_arch):?>
            <option value="<?=$proceso_arch['id']?>" <?=$proceso_arch['id']==$proceso->id?'selected':''?>><?=$proceso_arch['version']?></option>
        <?php endforeach ?>
    </select>    
</form>
<hr>
...
<div class="row-fluid acciones-generales">
    ...
    <div class="btn-group pull-left">
        ...
        <ul class="dropdown-menu">
            <li><a href="<?= site_url('backend/seguimiento/borrar_proceso/' . $proceso->id) ?>" onclick="if (confirm('¿Esta seguro que desea eliminar todos los trámites de este proceso?')){
            confirm('Atención. Esta operación no se podra deshacer y borrara todos los trámites en curso de este proceso. ¿Esta seguro que desea continuar?');
            return confirm('La operación se realizará en segundo plano, tenga en cuenta que esto puede demorar');}
        else
    ...
    <tbody>
        <?php foreach ($tramites as $t): ?>
            <tr>
                ...
                <td><?= $documento; ?></td>
                <td><?= strftime('%d %b %Y %H:%M:%S', mysql_to_unix($t->created_at)) ?></td>
                <td><?= strftime('%d %b %Y %H:%M:%S', mysql_to_unix($t->updated_at)) ?></td>
                <td class="actions">
