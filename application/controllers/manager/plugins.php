...
class Plugins extends CI_Controller {
    ...
    public function configurar_agenda(){

      $configurar_agenda = $this->input->post('configurar_agenda');
      $configurar_agenda_tipo = $this->input->post('configurar_agenda_tipo');

      if ($configurar_agenda_tipo == '1'){
        //es eliminar
        $output = shell_exec('crontab -l');
        $configurar_agenda = null;
        foreach(preg_split("/((\r?\n)|(\r\n?))/", $output) as $line){
            //si tiene el plgun de agenda configurado
            if (strpos($line, 'agenda conciliacion')){
               $agenda = 'Configurado';
               $configurar_agenda = $line;
            }
        }
        if ($configurar_agenda){
          $configurar_agenda = substr ( $configurar_agenda, strpos($configurar_agenda, 'php') );
          $command = "crontab -l | grep -v '". $configurar_agenda. "'  | crontab -";
          log_message('ERROR',$command);
          shell_exec($command);
        }


      }else if ($configurar_agenda_tipo == '2' && $configurar_agenda){
        //es crear
        shell_exec('(crontab -l 2>/dev/null; echo "'.$configurar_agenda .'") | crontab -');
      }
