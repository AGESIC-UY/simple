...
class Trazabilidad extends Doctrine_Record {
  ...
  public function perform() {
    ...
    $id_organismo = $this->args['id_organismo'];
    ...
    $id_transaccion = str_replace(" ", "_", strtoupper($id_organismo)) . ':' . str_replace(" ", "_", strtoupper($proceso_externo_id)) . ':' . $etapa->id . '.' . $usuario_id;
        ...
        <soapenv:Body>
          <lin:persist>
             <traza>
                ...
                <estadoLinea>'. ($secuencia == 0 ? 'INICIO' : ($secuencia == sizeof($etapa->getPasosEjecutables()) - 1 ? 'FIN' : 'EN_PROCESO')) .'</estadoLinea>
             </traza>
          </lin:persist>
        </soapenv:Body>
      </soapenv:Envelope>';
