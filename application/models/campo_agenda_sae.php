...
class CampoAgendaSae extends Campo{
    ...
    protected function display($modo, $dato, $etapa_id) {
      ...
      $display .=   '<div class="agenda-container">';
      ...
      $display .=         '<div class="span6">';
                            ...
                            foreach ($resultado->recursos as $recurso) {
                              $se_encontro_recurso_id = $id_recurso_con_variables && ((int)$recurso->id) == ((int)$id_recurso_con_variables);

                              if($se_encontro_recurso_id || !$id_recurso_con_variables){
                                if($recurso->latitud){
      ...
      $display .='<script type="text/javascript">
                  ...
                  function confirmar_reserva_click(){
                    ...
                    $.ajax({
                      type: "POST",
                      dataType: "json",
                      url: "'.  site_url("etapas/agenda_sae_api_confirmar_reserva").'",
                      data: {
                              "method": "POST",
                        ...
                        .done(function(data) {
                          ...
                          if(parseInt(data.resultado) === 1){
                            ...
                            $.ajax({
                              type: "POST",
                              dataType: "json",
                              url: "'.  site_url("etapas/confirmar_reserva_agenda_sae_interno_simple").'",
                              data: {
    ...
    public function backendExtraFields() {
      ...
      $display .= '<div class="control-group"><div><label for="datos_ws">Datos</label><textarea id="datos_ws" class="input-xxlarge" name="extra[datos_ws]"
      placeholder="agrupacion_campo,nombre_campo,valor_campo;agrupacion_campo,nombre_campo,valor_campo">'.$datos_ws.'</textarea></div></div>';
    ...
    public function backendExtraValidate() {
        ...
        $CI->form_validation->set_rules('extra[datos_ws]', 'Datos', 'trim|required');
    }
