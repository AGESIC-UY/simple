...
class Configuracion extends MY_BackendController {
    ...
    public function grupo_usuarios_editar($grupo_usuarios_id = NULL) {
        ...
        if ($grupo_usuarios_id) {
            ...
            $es_grupo_pre_definido = $grupo_usuarios->nombre == 'UsuarioMesaDeEntrada' || $grupo_usuarios->nombre == 'UsuarioFirmaPorLotes';
        }

        $data['es_grupo_pre_definido'] = $es_grupo_pre_definido;
    ...
    public function grupo_usuarios_editar_form($grupo_usuarios_id = NULL) {
        ...
        if ($grupo_usuarios) {
            ...
            $es_grupo_pre_definido = null;
        }
        if (!$es_grupo_pre_definido) {
            $this->form_validation->set_rules('nombre', 'Nombre', 'required');
        }

        $respuesta = new stdClass();

        if ($this->form_validation->run() == TRUE || $es_grupo_pre_definido) {
            $op = "update";
            if (!$grupo_usuarios) {
...
//            $usuarios = Doctrine::getTable('GrupoUsuarios')->find($grupo_usuarios_id)->Usuarios;
            try {
                $grupo_usuarios->save();
            } catch (Exception $exc) {
                $respuesta->validacion = FALSE;
                $respuesta->errores = "El grupo ya existe";
                echo json_encode($respuesta);
                exit();
            }
            auditar('GrupoUsuarios', $op, $grupo_usuarios->id, UsuarioBackendSesion::usuario()->usuario);
//            if ($op == "update") {
//                foreach ($usuarios as $usuario) {
//                    auditar('Usuario', "update", $usuario->id, UsuarioBackendSesion::usuario()->usuario);
//                }
//            }
            ...
            if (isset($grupo_usuarios_originales)) {
    ...
    public function grupo_usuarios_eliminar($grupo_usuarios_id) {
        ...
        auditar('GrupoUsuarios', "delete", $grupo_usuarios->id, UsuarioBackendSesion::usuario()->usuario);
//        $usuarios = Doctrine::getTable('GrupoUsuarios')->find($grupo_usuarios_id)->Usuarios;
...
//            $grupos_viejos = Doctrine::getTable('Usuario')->find($usuario_id)->GruposUsuarios;
            $usuario->save();
//            $grupos_nuevos = Doctrine::getTable('Usuario')->find($usuario_id)->GruposUsuarios;
            auditar('Usuario', $op, $usuario->id, UsuarioBackendSesion::usuario()->usuario);
//            if ($op == "update") {
//                foreach ($grupos_viejos as $grupo) {
//                    auditar('GrupoUsuarios', "update", $grupo->id, UsuarioBackendSesion::usuario()->usuario);
//                }
//                foreach ($grupos_nuevos as $grupo) {
//                    auditar('GrupoUsuarios', "update", $grupo->id, UsuarioBackendSesion::usuario()->usuario);
//                }
//            }
            //si existe este usuario en el backend con rol seguimiento
    ...
    // función para validar si un usuario tiene trámites asignados usuarios antes de eliminarlo
    public function ajax_tramites_pendiente(){
        ...
        $usuario = Doctrine::getTable('Usuario')->find($usuario_id);
        if ($usuario->cuenta_id != UsuarioBackendSesion::usuario()->cuenta_id) {
            $resultado->mensaje =  'No tiene permisos para eliminar este usuario.';
            
        } else {

            $tramites_pendiente = Doctrine_Query::create()
                                    ->from('Etapa e')
                                    ->where('e.usuario_id = ?', $usuario_id)
                                    ->andWhere('e.pendiente = ?', 1)
                                    ->count();
            $resultado->usuario = $usuario_id;
            $resultado->tramites_pendiente = $tramites_pendiente;
        }
...
/*
        if ($usuario->Etapas->count()) {
            ...
            auditar('Usuario', "delete", $usuario->id, UsuarioBackendSesion::usuario()->usuario);
 ...
 */
        // Cambios tomando en cuenta que ya se realizó la validación y confirmación al usuario 
        $this->verificar_seguimiento_grupo($usuario, true);
        
        // Liberar trámites antes de eliminar usuario, se apertura una transacción para evitar errores
        ...
        try {
            ...
            foreach($tramites_pendiente as $t){
                $etapa = Doctrine::getTable('Etapa')->find($t['id']);
                $etapa->usuario_original_id = $usuario_id;
                $etapa->usuario_original_historico = $etapa->usuario_original_id;
    ...
    public function backend_usuario_editar_form($usuario_id = NULL) {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $usuario->seg_alc_auditor = $this->input->post('seg_alc_auditor');

            $usuario->cuenta_id = UsuarioBackendSesion::usuario()->cuenta_id;
            $usuario->save();
            auditar('UsuarioBackend', $op, $usuario->id, UsuarioBackendSesion::usuario()->usuario);
            $respuesta->validacion = TRUE;
    ...
    public function backend_usuario_eliminar($usuario_id) {
        ...
        auditar('UsuarioBackend', "delete", $usuario->id, UsuarioBackendSesion::usuario()->usuario);
        $usuario->delete();
        ...
        foreach ($parametros as $parametro) {
            ...
            if ($parametro['id'] != 'null') {
                $parametro_obj = Doctrine_Query::create()
                        ->from('Parametro p')
                        ->where('p.cuenta_id = ? AND p.id = ?', array(UsuarioBackendSesion::usuario()->cuenta_id, $parametro['id']))
                        ->fetchOne();
            } else {
    ...
    public function monitoreo_estado_soap_pdi() {
        ...
        //-- filtros
        $busqueda_tipo_servicio = $this->input->get('busqueda_tipo_servicio');
        $busqueda_nombre_servicio = $this->input->get('busqueda_nombre_servicio');
        $busqueda_fecha_desde = $this->input->get('busqueda_fecha_desde');
        $busqueda_fecha_hasta = $this->input->get('busqueda_fecha_hasta');
        $busqueda_id_tramite = $this->input->get('busqueda_id_tramite');
        $busqueda_id_etapa = $this->input->get('busqueda_id_etapa');

        $data['busqueda_tipo_servicio'] = $busqueda_tipo_servicio;
        $data['busqueda_nombre_servicio'] = $busqueda_nombre_servicio;
        $data['busqueda_fecha_desde'] = $busqueda_fecha_desde;
        $data['busqueda_fecha_hasta'] = $busqueda_fecha_hasta;
        ...
        if ($busqueda_tipo_servicio || $busqueda_nombre_servicio || $busqueda_fecha_desde || $busqueda_fecha_hasta || $busqueda_id_tramite || $busqueda_id_etapa) {
            $data['filtro_aplicado'] = true;
        } else {
            $data['filtro_aplicado'] = false;
        }
        //-- termina filtros
        //-- paginado
        $this->load->helper('paginado_bd_helper');
        $offset = obtener_offset($this);
        $cantidad_elementos_por_pagina = obtener_cantidad_elementos_por_pagina();

        //consulta BD
        $resultado_query_monitoreo = Monitoreo::getListaOrdenadaIdPaginado($cantidad_elementos_por_pagina, $offset, $busqueda_tipo_servicio, $busqueda_nombre_servicio, $busqueda_fecha_desde, $busqueda_fecha_hasta, $busqueda_id_tramite, $busqueda_id_etapa);

        $url_controller = "backend/configuracion/monitoreo_estado_soap_pdi?busqueda_tipo_servicio={$busqueda_tipo_servicio}&busqueda_nombre_servicio={$busqueda_nombre_servicio}&busqueda_fecha_desde={$busqueda_fecha_desde}&busqueda_fecha_hasta={$busqueda_fecha_hasta}&busqueda_id_tramite={$busqueda_id_tramite}&busqueda_id_etapa={$busqueda_id_etapa}";
        $cantidad_total_elementos = $resultado_query_monitoreo->cantidad;
        paginar($this, $url_controller, $cantidad_total_elementos, $cantidad_elementos_por_pagina);
        $this->pagination ? $data['paginado'] = $this->pagination->create_links() : $data['paginado'] = ''; //carga div de paginado en la vista
        //-- termina paginado

        $data['lista_monitoreo'] = $resultado_query_monitoreo->datos;
    ...
    private function trazabilidad_ping_linea_monitoreo() {
        try {
            $soap_body_ping_linea = '<soapenv:Envelope
            xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/"
            xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    public function etiquetas() {
        ...
        $data['title'] = 'Etiquetas de trazabilidad';
        $data['content'] = 'backend/configuracion/etiquetas';
        $this->load->view('backend/template', $data);
    }

    public function etiquetas_form() {
        $etiquetas = $this->input->post('etiqueta');

        foreach ($etiquetas as $etiqueta) {
            if (empty($etiqueta['etiqueta'])) {
                $respuesta = new stdClass();
                $respuesta->validacion = false;
                $respuesta->errores = '<div class="alert alert-error">El campo <strong>etiqueta</strong> no puede estar vacío.</div>';
                $respuesta->redirect = site_url('backend/configuracion/etiquetas');
                echo json_encode($respuesta);
                exit;
            }

            if ($etiqueta['id'] != 'null') {
                $etiqueta_obj = Doctrine_Query::create()
                        ->from('EtiquetaTraza et')
                        ->where('et.id = ?', strtoupper($etiqueta['id']))
                        ->fetchOne();
            } else {
                $etiqueta_obj = new EtiquetaTraza();
            }
            $etiqueta_obj->etiqueta = strtoupper($etiqueta['etiqueta']);
            $etiqueta_obj->descripcion = $etiqueta['descripcion'];
            $etiqueta_obj->save();
        }
    ...
    public function eliminar_etiquetas() {
        $etiqueta_id = $this->input->post('etiqueta_id');

        $etiqueta = Doctrine::getTable('EtiquetaTraza')->find($etiqueta_id);
        if ($etiqueta) {
            $etiqueta->delete();
        }
    }
    ...
    public function involucrado() {
        ...
        $data['content'] = 'backend/configuracion/trazabilidad_involucrado';
        $this->load->view('backend/template', $data);
    }
    
    public function trazabildiad_involucrado_form() {  
        $respuesta = new stdClass();
        ...
        if ($involucrado) {
            $this->form_validation->set_rules('trazabilidad_involucrado', 'Involucrado', 'required');
             if ($this->form_validation->run() == true) {                
                $cuenta = Doctrine::getTable('Cuenta')->find(UsuarioBackendSesion::usuario()->cuenta_id);
                $cuenta->traza_involucrado = $involucrado;
                $cuenta->save();

                $respuesta->validacion = true;
                $respuesta->redirect = site_url('backend/configuracion/involucrado');
            } else {
                $respuesta->validacion = false;
                $respuesta->errores = validation_errors();
            }
