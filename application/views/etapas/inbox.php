<?php if(!isset($funcionario_actuando_como_ciudadano) || (isset($funcionario_actuando_como_ciudadano) && !$funcionario_actuando_como_ciudadano)):?>
  <h1>Trámites en bandeja de entrada</h1>
  ...
  <?php else:?>
        <h1>Trámites en bandeja de entrada <b class="blue"> - Ejecutando como ciudadano: <?php echo $usuario_nombres;?></b></h1>
        ...
        <?php foreach ($etapas as $e): ?>
            <tr <?=$e->getPrevisualizacion()?'data-toggle="popover" data-html="true" data-title="<h4>Previsualización</h4>" data-content="'.htmlspecialchars($e->getPrevisualizacion()).'" data-trigger="hover" data-placement="bottom"':''?>>
                ...
                <td class="actions" data-title="Acciones">
                    ...
                    <?php if(count($e->Tramite->Etapas)==1 && eliminarTramite($e->tramite_id)):?><a href="<?=site_url('tramites/eliminar/'.$e->tramite_id)?>" class="btn btn-danger" onclick="return confirm('¿Esta seguro que desea eliminar este tramite?')"><span class="icon-trash"></span><span class="hide-text">Eliminar <?= $e->Tramite->Proceso->nombre ?></span></a><?php endif ?>
                </td>
            </tr>
        <?php endforeach; ?>
    </tbody>
