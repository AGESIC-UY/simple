...
<?php else: ?>
    ...
    <script type="text/javascript" src="<?= base_url() ?>assets/js/seguimiento_frontend.js"></script>
<?php endif ?>
...
<ul id="skip">
    <li><a href="#areaDibujo">Ir al contenido</a></li>
</ul>

<h1><?= $tramite->Proceso->nombre ?></h1>
<ul class="breadcrumb">
    <li><a href="<?= site_url('historico/ver/' . $tramite->id) ?>"><?= $tramite->Proceso->nombre ?></a><span class="divider">/</span></li>
    <li class="active">Trámite # <?= $tramite->id ?></li>
</ul>


<div id="areaDibujo">
    <h2>Trámite # <?= $tramite->id ?></h2>
            ...
            <?php foreach ($etapas as $e): ?>
                <li>
                    ...
                    <?php if ($e->canUsuarioRevisarDetalleFrontend(UsuarioSesion::usuario())) : ?>
                        <p><a href="<?= site_url('historico/ver_etapa/' . $e->id) ?>">Revisar detalle<span class="hidden-accessible">-<?= mt_rand() ?></span></a></p>
                    <?php endif ?>
                </li>
            <?php endforeach; ?>
        </ul>
