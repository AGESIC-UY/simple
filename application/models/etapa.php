...
class Etapa extends Doctrine_Record {
    ...
    function setUp() {
        ...
        $this->hasMany('ObnDatosSeguimiento as ObnDatosSeguimiento', array(
            'local' => 'id',
            'foreign' => 'etapa_id'
        ));
    }
    ...
    // se creo para la api operacion "instanciar"
    public function avanzar_sin_ejecutar_eventos($usuarios_a_asignar = null) {
        Doctrine_Manager::connection()->beginTransaction();
        // Cerramos esta etapa
        $this->cerrar_sin_ejecutar_eventos();

        $tp = $this->getTareasProximas();
        if ($tp->estado != 'sincontinuacion') {
            if ($tp->estado == 'completado') {
                if ($this->Tramite->getEtapasActuales()->count() == 0)
                    $this->Tramite->cerrar_sin_ejecutar_eventos();
            }
            else {
                if ($tp->estado == 'pendiente') {
                    $tareas_proximas = $tp->tareas;
                    foreach ($tareas_proximas as $tarea_proxima) {
                        ...
                        if ($usuario_asignado_id)
                            $etapa->asignar_sin_ejecutar_eventos($usuario_asignado_id);
    ...
    // se creo para la api operacion "instanciar"
    public function asignar_sin_ejecutar_eventos($usuario_id) {
        if (!$this->canUsuarioAsignarsela($usuario_id))
            return;
    ...
    public function asignar_sin_validacion($usuario_id) {
        $this->usuario_id = $usuario_id;
        $this->save();
    }
    ...
    public function cerrar() {
        ...
        foreach ($eventos as $e) {
            $r = new Regla($e->regla);
            if ($r->evaluar($this->id)) {
                //evento que se ejecutan despues de la tarea si falla se ignora siempre se cierra
                $e->Accion->ejecutar($this, $e);
                //if ($e->Accion->ejecutar($this, $e) != null && $e->Accion->ejecutar($this, $e) != false) return;
            }
        }

        $validaciones = Doctrine_Query::create()->from('EjecutarValidacion ev')
                ->where('ev.paso_id = ? AND ev.instante = ?', array($this->Tarea->id, 'antes'))
                ->execute();
        foreach ($validaciones as $v) {
            $r = new Regla($v->regla);
            if ($r->evaluar($this->id))
                $v->Validacion->ejecutar($this);
        }
    ...
    public function calcularVencimiento() {
        ...
        if ($this->Tarea->vencimiento_a_partir_de_variable) { //vencimiento a partir de variable o fecha (d-m-a)
            $regla = new Regla($this->Tarea->vencimiento_a_partir_de_variable);
            $fecha_variable = $regla->getExpresionParaOutput($this->id);

            if (!$fecha_variable) {
                return NULL;
            }

            $fecha_variable = date("Y-m-d", strtotime($fecha_variable));

            if ($this->Tarea->vencimiento_unidad == 'D') {
                if ($this->Tarea->vencimiento_habiles) {
                    $fecha = add_working_days($fecha_variable, $this->Tarea->vencimiento_valor);
                } else {
                    $temp = new DateTime($fecha_variable);
        ...
        } else { //vencimiento a partir de una atapa anterior
            if ($this->Tarea->vencimiento_unidad == 'D') {
                if ($this->Tarea->vencimiento_habiles) {
    ...
    public function getFechaVencimientoAsString() {
        if ($this->Tarea->vencimiento_a_partir_de_variable) {
            //se debe recalcular la fecha para las etapas con fecha variable
            $this->vencimiento_at = $this->calcularVencimiento();
            $this->save();
        }
        ...
        if ($interval->invert) {
            return 'vencida';
        } else if ($this->Tarea->vencimiento_a_partir_de_variable) {
            $regla = new Regla($this->Tarea->vencimiento_a_partir_de_variable);
            $fecha_variable = $regla->getExpresionParaOutput($this->id);
            ...
            if ($fecha_variable) {
                if ($interval->d == 0 && $interval->m == 0 && $interval->y == 0) {
                    ...
                    if ($interval->d != 0)
                        $mensaje = $interval->d . ' día/s ';

                    if ($interval->m != 0)
                        $mensaje .= 'y ' . $interval->m . ' mes/es ';

                    if ($interval->y != 0)
                        $mensaje .= 'y ' . $interval->y . ' año/os ';

                    return 'vence en ' . $mensaje . ' a partir hoy';
                }
            }
            else {
                return 'vence en ' . ($this->Tarea->vencimiento_valor) . ' día/s después de la fecha definida en el campo "' . str_replace('@@', '', $this->Tarea->vencimiento_a_partir_de_variable) . '" del formulario';
            }
        } else {
            if ($interval->d == 0 && $interval->m == 0 && $interval->y == 0) {
                return 'vence hoy';
    ...
    public function iniciarPaso(Paso $paso, $secuencia) {
        ...
        $validaciones = Doctrine_Query::create()->from('EjecutarValidacion ev')
                ->where('ev.paso_id = ? AND ev.instante = ?', array($paso->id, 'antes'))
                ->execute();
        foreach ($validaciones as $v) {
            $r = new Regla($v->regla);
            if ($r->evaluar($this->id))
    ...
    public function finalizarPaso(Paso $paso, $secuencia) {
        ...
        $validaciones = Doctrine_Query::create()->from('EjecutarValidacion ev')
                ->where('ev.paso_id = ? AND ev.instante = ?', array($paso->id, 'despues'))
                ->execute();
    ...
    public function actualizarTarea($proceso_id) {
        $proceso = Doctrine::getTable('Proceso')->find($proceso_id);
        if ($proceso) {
            $tarea_actual = $this->Tarea;
            $tarea_nueva = Doctrine::getTable('Tarea')->findByProcesoId($proceso->id);
            foreach ($tarea_nueva as $tarea) {
                if ($tarea->id_x_tarea == $tarea_actual->id_x_tarea) {
