...
$configura_agenda= '0 2 * * * php DIRECTORIO_SIMPLE/index.php tasks/agenda conciliacion > RUTA_LOG/nombre.log';
foreach(preg_split("/((\r?\n)|(\r\n?))/", $output) as $line){
    //si tiene el plgun de pagos configurado
    if (strpos($line, 'agenda conciliacion')){
       $agenda = 'Configurado';
       $configura_agenda = $line;
    }
...
<table class="table">
  <caption class="hide-text">Cuentas</caption>
    ...
    <tbody>
        ...
        <tr>
          <form method="POST" action="<?=site_url('manager/plugins/configurar_agenda')?>">
            <td>Tarea de Agenda (conciliacion)</td>
            <td><?=posix_getpwuid(posix_geteuid())['name']?></td>
            <td><?=$agenda ?></td>
            <?php if ($agenda == 'Configurado') { ?>
              <td><?=$configura_agenda ?></td>
            <?php } ?>
            <?php if ($agenda == 'No Configurado') { ?>
              <td><input  name="configurar_agenda" id="configurar_agenda" type='text' value='<?=$configura_agenda ?>'></input></td>
            <?php } ?>
            <td class="actions">
              <?php if ($agenda == 'Configurado') { ?>
                <input style='display:none' name="configurar_agenda_tipo" id="configurar_agenda_tipo" type='text' value='1'></input>
                <button class="btn btn-primary btn-lg" type="submit">Eliminar</button>
              <?php } ?>
              <?php if ($agenda == 'No Configurado') { ?>
                <input style='display:none' name="configurar_agenda_tipo" id="configurar_agenda_tipo" type='text' value='2'></input>
                <button class="btn btn-primary btn-lg" type="submit">Configurar</button>
              <?php } ?>
