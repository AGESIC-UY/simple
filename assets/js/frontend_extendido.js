...
$(document).ready(function () {
    $('#btn_siguiente_firma_lote').click(function (event) {
        event.preventDefault();
        var contenedor = $("#mainTable").find("tbody").find("tr");
        var form = $(".ajaxForm");
        var limite = $("#limite_firma_lote").val();
        var lista = "[";
        var cantidad = 0;
        ...
        contenedor.each(function (index) {
            if ($(contenedor[index]).find('td').find("input").attr('checked')) {
                lista += "{" + "\"id_tramite\":" + $(contenedor[index]).find('td')[1].innerHTML + "," + "\"id_etapa\":" + $(contenedor[index]).find('td')[2].innerHTML + "},";
                cantidad++;
            }
        });
        
        if (cantidad > limite) {
            $("<div title='Ha ocurrido un error' class='validacion-popup validacion-error'><div class='dialog-icon'><span class='icn icn-circle-error-lg'></span></div>Ha excedido el límite de selección</div>").dialog(
        ...
        if (cantidad == 0) {
            $("<div title='Ha ocurrido un error' class='validacion-popup validacion-error'><div class='dialog-icon'><span class='icn icn-circle-error-lg'></span></div>Debe seleccionar un trámite para firmar</div>").dialog(
                    {modal: true, draggable: false,
                        resizable: false,
                        dialogClass: 'ui-dialog-tabla',
                        buttons: {
        ...
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: site_url + 'tramites/firmar_documentos_lotes',
            data: {
                'lista': lista
            },
            async: true,
            complete: function (resul) {
                try {
                    var resultado = JSON.parse(resul.responseText);
                    if (resultado.error) {
                        $("<div title='Ha ocurrido un error' class='validacion-popup validacion-error'><div class='dialog-icon'><span class='icn icn-circle-error-lg'></span></div>" + resultado.msg + "</div>").dialog(
                                {modal: true, draggable: false,
                                    resizable: false,
                                    dialogClass: 'ui-dialog-tabla',
                                    buttons: {
                        ...
                        $(ajaxLoader).remove();
                        $.unblockUI();
                    } else if (resultado.jnlp) {
                        var filename = "firma_lote.jnlp";
                        var text = resultado.jnlp;
                        var blob = new Blob([text], {type: 'application/jnlp'});
                        var link = document.getElementsByName('download_jnlp')[0];
                        if (!link) {
                            link = document.createElement("a");
                            link.download = filename;
                            link.innerHTML = "Download File";
                            link.name = 'download_jnlp';
                            link.target = '_blank';
                        ...
                        link.click();
                        $("#btn_finalizar_firma_lote").prop("disabled", false);
                        var ambos = document.getElementsByName('doc_ambos')[0];
                        if (!ambos) {
                            ambos = document.createElement("input");
                            ambos.type = 'text';
                            ambos.style = 'display: none';
                            ambos.name = 'doc_ambos';
                        }
                        ambos.value = resultado.files_ambos;
                        document.body.appendChild(ambos);
                        var organismo = document.getElementsByName('doc_organismo')[0];
                        if (!organismo) {
                            organismo = document.createElement("input");
                            organismo.type = 'text';
                            organismo.style = 'display: none';
                            organismo.name = 'doc_organismo';
                        }
                        organismo.value = resultado.files_organismo;
                        document.body.appendChild(organismo);
                        var funcionario = document.getElementsByName('doc_funcionario')[0];
                        if (!funcionario) {
                            funcionario = document.createElement("input");
                            funcionario.type = 'text';
                            funcionario.style = 'display: none';
                            funcionario.name = 'doc_funcionario';
                        }
                        funcionario.value = resultado.files_funcionario;
                        document.body.appendChild(funcionario);
                        $(ajaxLoader).remove();
                    } else if (resultado) {
                        $("#btn_finalizar_firma_lote").prop("disabled", false);
                        var ambos = document.getElementsByName('doc_ambos')[0];
                    ...
                    $("<div title='Ha ocurrido un error' class='validacion-popup validacion-error'><div class='dialog-icon'><span class='icn icn-circle-error-lg'></span></div>No se ha podido firmar el documento</div>").dialog(
                            {modal: true, draggable: false,
                                resizable: false,
                                dialogClass: 'ui-dialog-tabla',
                                buttons: {
    ...
    $('#btn_finalizar_firma_lote').click(function (event) {
        event.preventDefault();
        var doc_funcionario = document.getElementsByName('doc_funcionario')[0].value;
        var doc_organismo = document.getElementsByName('doc_organismo')[0].value;
        var doc_ambos = document.getElementsByName('doc_ambos')[0].value;
        var form = $(".ajaxForm");
        $(form).append("<div class='ajaxLoader'>Cargando</div>");
        var ajaxLoader = $(form).find(".ajaxLoader");
        $(ajaxLoader).css({
        ...
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: site_url + 'tramites/confirmar_firma_por_lotes',
            data: {
                'doc_funcionario': doc_funcionario,
                'doc_organismo': doc_organismo,
                'doc_ambos': doc_ambos
            },
            async: true,
            complete: function (resultado) {
                ...
                if (resultado.error) {
                    ...
                    $.when(export_CSV(doc_funcionario, doc_organismo, doc_ambos)).then(function () {
                    });
                }
            }
        });
        //  window.location.href = $.post( site_url + 'tramites/resumen_firma_por_lotes', {   'doc_funcionario': doc_funcionario,'doc_organismo':doc_organismo,'doc_ambos': doc_ambos } );
    });

    $('#btn_update_firma_lote').click(function (event) {
        event.preventDefault();
    ...
    $('#generar_reporte_completo').click(function (e) {
        ...
        $.ajax({
            type: 'post',
            url: site_url + 'tramites/generar_completo/' + reporte_id
                    + "?filtro_grupo=" + grupo
            ...
            success: function (response) {
                ...
                if (resultado.error) {
                    ...
                    $('#mensaje_limite_completo').hide();
                    $('#error').remove();
                    window.location.href = site_url + "tramites/ver_reporte/" + reporte_id
                            + "?filtro_grupo=" + grupo
                            + "&filtro_usuario=" + usuario
                            + "&filtro_desde=" + desde
                            + "&filtro_hasta=" + hasta
    ...
    $('#generar_reporte_basico').click(function (e) {
        ...
        if (desde == "" || hasta == "") {
            ...
            $.ajax({
                type: 'post',
                url: site_url + 'tramites/generar_basico/' + reporte_id
                        + "?&filtro_desde=" + desde
                        + "&filtro_hasta=" + hasta
                        + "&desdeCambio=" + desdeCambio
                        + "&hastaCambio=" + hastaCambio
    ...
    $('#generar_reporte_completo_email').click(function (e) {
        ...
        $.ajax({
            type: 'post',
            url: site_url + 'tramites/ver_completo_email/' + reporte_id
                    + "?filtro_grupo=" + grupo
                    + "&filtro_usuario=" + usuario
                    + "&filtro_desde=" + desde
                    + "&filtro_hasta=" + hasta
    ...
    $('#generar_reporte_basico_email').click(function (e) {
        ...
        $.ajax({
            type: 'post',
            url: site_url + 'tramites/ver_basico_email/' + reporte_id
                    + "?filtro_desde=" + desde
                    + "&filtro_hasta=" + hasta
                    + "&email=" + email
                    + "&desdeCambio=" + desdeCambio
    ...
    if (window.location.href.split('?')[0] === document.Constants.host + '/tramites/busqueda_filtros_participados') {
        ...
        $('#busqueda_filtro').css('display', 'block');
    } else if (window.location.href.split('?')[0].includes(document.Constants.host + '/tramites/firma_por_lotes') && window.location.href.split('?').length > 1) {
        if (!window.location.href.split('?')[1].includes('filtro')) {
            if (window.location.href.split('?')[1].split('&')[1].includes('offset')) {
    ...
    //-- Nuevos Filtros por base de datos Firma
    $('#btn_buscar_filtro_firma').click(function (e) {
        e.preventDefault();
        var busqueda_id_tramite = $('#busqueda_id_tramite').val();
        var busqueda_id_etapa = $('#busqueda_id_etapa').val();
        var busqueda_nombre_tarea = $('#busqueda_nombre_tarea').val();
        var busqueda_modificacion_desde = $('#busqueda_modificacion_desde').val();
        var busqueda_modificacion_hasta = $('#busqueda_modificacion_hasta').val();
        var id_proceso = $('#id_proceso').val();

        var url_datos_firma = '/tramites/firma_por_lotes/' + id_proceso;
        $.ajax({
            type: 'POST',
            dataType: 'html',
            url: document.Constants.host + url_datos_firma,
            data: {
                'busqueda_id_tramite': busqueda_id_tramite,
                'busqueda_id_etapa': busqueda_id_etapa,
                'busqueda_nombre_tarea': busqueda_nombre_tarea,
                'busqueda_modificacion_desde': busqueda_modificacion_desde,
                'busqueda_modificacion_hasta': busqueda_modificacion_hasta
            },
            async: true,
    ...
    $('.consulta_estado_pago').click(function (e) {
        ...
        $.ajax({
            ...
            data: {'IdSol': $('input[name="IdSol"]').val(), 'IdTramite': $('input[name="IdTramite"]').val(), 'IdEtapa': $('input[name="IdEtapa"]').val(), 'IdPasarela': $('#pasarela_generica_id_antel').val()},
            complete: function (resultado) {
...
function export_CSV(doc_funcionario, doc_organismo, doc_ambos) {
    $.ajax({
        type: 'post',
        url: document.Constants.host + '/etapas/exportCSV',
        data: {
            'doc_funcionario': doc_funcionario,
            'doc_organismo': doc_organismo,
            'doc_ambos': doc_ambos
        ...
        complete: function (resultado) {
            var blob = new Blob([resultado.responseText], {type: 'application/csv'});
            var link = document.getElementsByName('download_csv')[0];
            if (!link) {
                link = document.createElement("a");
                link.download = "resumen_firma.csv";
                link.innerHTML = "Download File";
                link.name = 'download_csv';
                link.target = '_blank';
                link.style = 'display: none';
            }
            document.body.appendChild(link);
            link.href = window.URL.createObjectURL(blob);
            link.click();
            $("#btn_finalizar_firma_lote").prop("disabled", true);
            var form = $(".ajaxForm");
            var ajaxLoader = $(form).find(".ajaxLoader");
            $(ajaxLoader).remove();
            var doc_funcionario = document.getElementsByName('doc_funcionario')[0];
            $(doc_funcionario).remove();
            var doc_organismo = document.getElementsByName('doc_organismo')[0];
            $(doc_organismo).remove();
            var doc_ambos = document.getElementsByName('doc_ambos')[0];
            $(doc_ambos).remove();
            $.unblockUI();
            window.location.reload();
            return true;
