...
class sspmod_saml_Auth_Source_SP extends SimpleSAML_Auth_Source {
	...
	private function startSSO2(SimpleSAML_Configuration $idpMetadata, array $state) {
		...
		$ar->setAssertionConsumerServiceURL($this->config['AssertionConsumerServiceURL']);
		$ar->setConsent($this->config['Consent']);
		$ar->setForceAuthn((bool)$this->config['ForceAuthn']);
		$ar->setProviderName($this->config['ProviderName']);
		$ar->setAttributeConsumingServiceIndex($this->config['AttributeConsumingServiceIndex']);
	...
	public function startSLO2(&$state) {
		...
		*/
		...
		$nameId = array("Value" => $cookie_data[1]); //$state['saml:logout:NameID'];
		$sessionIndex = $cookie_data[0]; //$state['saml:logout:SessionIndex'];
	...
	public function logout(&$state) {
		...
		$logoutType = 'saml2'; //$state['saml:logout:Type'];
		switch ($logoutType) {
		case 'saml1':
			/* Nothing to do. */
			return;
