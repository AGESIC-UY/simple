...
class CampoPagos extends Campo {
    ...
    public function formValidate($etapa_id = null) {
        ...
        //el pago por parte del funcionario
        ...
        //validacion campo pago reuqerido mesa de entrada
        if (isset($this->extra->pago_requerido_online) && $this->extra->pago_requerido_online == 1 && !$pago && $funcionario_actuando_como_ciudadano && $this->pago_online == 0 && UsuarioSesion::usuarioMesaDeEntrada() && $CI->session->userdata('id_usuario_ciudadano') && $CI->uri->segment(1) != 'backend') {
            $CI->form_validation->set_rules('check_pago_online', MENSAJE_PAGO_FUNCIONARIO, 'required');
        }
    }
    ...
    protected function display($modo, $dato, $etapa_id) {
        ...
        if ($funcionario_actuando_como_ciudadano && $this->pago_online == 0 && UsuarioSesion::usuarioMesaDeEntrada() && $CI->session->userdata('id_usuario_ciudadano') && $CI->uri->segment(1) != 'backend') {
            //si actuó el funcionario y el usaurio logueado es el funcionario actuando como el ciudadano
            if (!$pago) {
                //capaz que ya pago el usuario de forma online entonces se deberia verificar
                //plantear incluir el boton de validacion de pago

                return '<label class="checkbox" for="check_pago_online"><input type="checkbox" name="check_pago_online" id="check_pago_online" value="1">' . MENSAJE_PAGO_FUNCIONARIO . '</label>';
            } else {

                return '<strong class="green">' . MENSAJE_PAGO_CONFIRMADO_FUNCIONARIO . '</strong><br>
              <label class="checkbox" for="check_cancelar_pago">
              <input type="checkbox" name="check_cancelar_pago" id="check_cancelar_pago" value="1"><b class="red">' . MENSAJE_PAGO_FUNCIONARIO_CANCELAR . '</label></b>';
            }
        } else if ($funcionario_actuando_como_ciudadano && $pago && !$CI->session->userdata('id_usuario_ciudadano') && $CI->uri->segment(1) != 'backend') {
            //si actuó el funcionario está pago por el funcionario y el usuario logueado no es el funcionario actuando como el ciudadano
            return '<strong class="green">' . MENSAJE_PAGO_CONFIRMADO_FUNCIONARIO . '</strong>';
        } else if ($funcionario_actuando_como_ciudadano && $CI->uri->segment(3) == 'ver_etapa') {
            //si actuó el funcionario y estoy en ver
            if (!$pago) {
                return '<strong class="black">' . MENSAJE_PAGO_NO_CONFIRMADO_FUNCIONARIO . '</strong>';
            } else {
                return '<strong class="green">' . MENSAJE_PAGO_CONFIRMADO_FUNCIONARIO . '</strong>';
            }
        } else {
            //caso normal sin actuar el funcionario
            if ($etapa_id) {
                ...
                case 'generico':
                    ...
                    if (strtolower($pasarela_generica->metodo_http) == 'post') {
                        ...
                        if (!filter_var($pasarela_generica->url_redireccion, FILTER_VALIDATE_URL)) {
                            ...
                            $url_redireccion = ($url_redireccion) ? $url_redireccion->valor : "";
                        } else {
                        ...
                        if ($dato) {
                            if ($dato->valor != '0') {
                                if ($dato->valor == '-1') {
                                    ...
                                    if ($modo == 'visualizacion') {
                                        $display .= '<input type="hidden" id="verificar_estado_pago_generico_visualizacion" />';
                                    } else {
                                    ...
                                    $display .= '<br /><br />';
                                    $display .= '<input id="' . $this->id . '" type="hidden" name="' . $this->nombre . '" value="' . $pasarela_generica->id . '" class="hidden" />';
                                    // $display .= '<input type="hidden" id="verificar_estado_pago_generico" />';
                                    $display .= '<input type="hidden" id="id_solicitud_pago_generico" value="' . $id_sol->valor . '" />';
                                    $display .= '<input type="hidden" id="etapa_id" value="' . $etapa_id . '" />';
                                    $display .= '<input type="hidden" id="pasarela_generica_id" value="' . $pasarela_generica->id . '" />';
                            ...
                            //$display .= '<input type="hidden" id="verificar_estado_pago_generico" />';
                            $display .= '<input type="hidden" id="id_solicitud_pago_generico" value="' . (($id_sol) ? $id_sol->valor : "") . '" />';
                            $display .= '<input type="hidden" id="etapa_id" value="' . $etapa_id . '" />';
                            $display .= '<input type="hidden" id="pasarela_generica_id" value="' . $pasarela_generica->id . '" />';
                            $display .= '<input name="MsgPagoGenerico" value="' . $this->etiqueta . '" type="hidden" />';
                            $display .= '<input id="var_idsol" value="' . $pasarela_generica->variable_idsol . '" type="hidden" />';
                    ...
                    $display .= '<div class="vista_componente_pago_generico hidden">';
                    $display .= '<div class="no-margin-box">';
                    $display .= '<div class="text-center">';
                    ...
                    if ($dato) {
                        if ($dato->valor != '0') {
                            ...
                            if ($dato->valor == '-1') {
                                ...
                                if ($modo == 'visualizacion') {
                                    $display .= '<input type="hidden" id="verificar_estado_pago_visualizacion" />';
                                } else {
                                ...
                                //la variabel codigo_estado_solicitud_pago esta en 1 NO SE verifica el esstado del pago se sabe que se tiene que volver a pagar
                                $error_servicio_pagos = Doctrine::getTable('DatoSeguimiento')->findOneByNombreAndEtapaId("ws_error", $etapa_id);
                                ...
                                $display .= '<div class="mensaje_estado_pago hidden"><div class="dialogo validacion-warning"><h3 class="dialogos_titulo small-loader">' . MENSAJE_PAGO_CONSULTA_TITULO . '</h3><div id="floatingCirclesG" class="small-loader"><div class="f_circleG" id="frotateG_01"></div><div class="f_circleG" id="frotateG_02"></div><div class="f_circleG" id="frotateG_03"></div><div class="f_circleG" id="frotateG_04"></div><div class="f_circleG" id="frotateG_05"></div><div class="f_circleG" id="frotateG_06"></div><div class="f_circleG" id="frotateG_07"></div><div class="f_circleG" id="frotateG_08"></div></div><div>' . MENSAJE_PAGO_CONSULTA . '</div></div></div>';
                                $display .= '<input name="IdSol" value="' . (($id_sol) ? $id_sol->valor : "" ) . '" type="hidden" />';
                                $display .= '<input name="IdEtapa" value="' . $etapa_id . '" type="hidden" />';
                            ...
                            $display .= '<input type="hidden" id="pasarela_generica_id_antel" value="' . $pasarela->pasarela_pago_id . '" />';
                            $display .= '<input name="IdEtapa" value="' . $etapa_id . '" type="hidden" />';
                            $display .= '<input name="MsgPago" value="' . $this->etiqueta . '" type="hidden" />';
                        ...
                        $display .= '<input name="IdTramite" value="' . (isset($etapa->tramite_id)?$etapa->tramite_id: "") . '" type="hidden" />';
                        $display .= '<input name="IdEtapa" value="' . $etapa_id . '" type="hidden" />';
                        $display .= '<input type="hidden" id="pasarela_generica_id_antel" value="' . $pasarela->pasarela_pago_id . '" />';
                        $display .= '<input name="MsgPago" value="' . $this->etiqueta . '" type="hidden" />';
    ...
    public function backendExtraFields() {
        ...
        if (isset($this->extra->pago_requerido_online) && (bool) $this->extra->pago_requerido_online) {
            $requerido_online_valor_check = '0';
            $checked = 'checked';
        } else {
            $requerido_online_valor_check = '1';
            $checked = '';
        }

        if ((bool) $this->pago_online) {
            $style = 'display:none';
        } else {
            $style = 'display:block';
        }

        $display = '<label id="label_pago_requerido" class="checkbox" for="pago_requerido_online" style="' . $style . '">
                  <input type="checkbox" name="extra[pago_requerido_online]" id="pago_requerido_online" value="' . $requerido_online_valor_check . '" ' . $checked . '>' . TEXTO_CONFIG_PAGO_REQUERIDO_ONLINE . '
                  </label>';

        $display .= '<script>
                    $("#check_pago_online").change(function() {
                      if($(this).val() == "0"){
                        $("#label_pago_requerido").show();
                      }
                      else{
                        $("#label_pago_requerido").hide();
                      }
                    });
                    </script>';
