...
class Pagos extends CI_Controller {
  ...
  public function conciliacion() {
    ...
    echo 'Iniciando proceso de conciliacion de pagos, aguarde por favor. (versión 1.4).' . PHP_EOL;
            ...
            if($campo->tipo == 'pagos') {
              ...
              if ($pago_fila){
                if ($pago_fila->estado == 'realizado'){
                  ...
                  echo 'Etapa id ' . $etapa->id . ' con pago id ' .  $pago_fila->id . ' en estado ' . $pago_fila->estado .  ' SE CIERRA '.  PHP_EOL;
                  $this->cerrarEtapa($etapa);
                } else if ( $pago_fila->id_solicitud > 0 && ($pago_fila->estado == 'pendiente'
                  || $pago_fila->estado == 'iniciado'  || $pago_fila->estado == 'token_solicita' )){
                    //invoca WS
                    $clave_tramite = Doctrine::getTable('DatoSeguimiento')->findOneByNombreAndEtapaId(md5($pago_fila->id . '_clave_tramite_pasarela_pagos'), $etapa->id);
                    ...
                    if($curl_errno > 0 || $http_code != 200) {
                      ...
                      switch($nuevo_estado[0]) {
                        ...
                        case '6':
                          ...
                          if(!empty($pago_fila)) {
                            $pago_fila->estado = 'rc';
                            $pago_fila->fecha_actualizacion = date('d/m/Y H:i');
                            $pago_fila->save();
                          }
                          $this->generar_variables_pago($pago_fila,$xml,$pago_fila->id_solicitud,$etapa->id);
                          $this->cerrarEtapa($etapa);
