...
class AccionWebserviceExtended extends Accion {
    ...
    public function ejecutar(Etapa $etapa, $evento = null) {
        $fecha_ejecucion_servicio =  date('Y-m-d', time()).' '.date('H:i:s', time());
        ...
        if($servicio->tipo == 'soap') {
          ...
          **/
          ...
          try {
              ...
              $hubo_error = $this->procesar_errores_soap($etapa, $curl_errno,$curl_error,$http_code, $soap_body,$soap_response,$xml,$soap_endpoint_location,$servicio,$fecha_ejecucion_servicio);
          ...
          **/
          ...
          try {
            ...
            log_message('error', "INICIO Token");
            log_message('error', $soap_operacion);
log_message('error', json_encode($header));
log_message('error', $soap_response);
log_message('error', $curl_errno);
log_message('error', $curl_error);
log_message('error', $http_code);
log_message('error', "FIN Token");
            ...
            $hubo_error = $this->procesar_errores_pdi($etapa, $curl_errno,$curl_error,$http_code, $body,$soap_response,$xml,$servicio,$pdi,$fecha_ejecucion_servicio);
            if ($hubo_error){
              $this->trazar($etapa, $evento);
              return true;
            }
...
log_message('error', $soap_operacion);
log_message('error', json_encode($header));
log_message('error', json_encode($service_soap));
log_message('error', $soap_response);
log_message('error', $curl_errno);
log_message('error', $curl_error);
log_message('error', $http_code);
            ...
            $hubo_error = $this->procesar_errores_pdi($etapa, $curl_errno,$curl_error,$http_code, $service_soap,$soap_response,$xml,$servicio,$pdi,$fecha_ejecucion_servicio);
            if ($hubo_error){
              $this->trazar($etapa, $evento);
              return true;
            }
        ...
        if($servicio->tipo == 'soap'){
          $this->guardar_monitoreo(
                                    $soap_endpoint_location, //$url_web_service
                                    $servicio->tipo, //$tipo
                                    '', //$rol - rol no se usa por ser soap
                                    '', //$certificado - certificado no se usa por ser soap
                                    'OK', //$error_texto
                                    false, //$error
                                    $soap_body, //$soap_peticion
                                    $soap_response, //$soap_respuesta
                                    $servicio->id, //$catalogo_id
                                    $servicio->requiere_autenticacion, //$seguridad
                                    $etapa,
                                    $fecha_ejecucion_servicio
                                  );
        }
        else {
          $this->guardar_monitoreo(
                                    $servicio->url_fisica, //$url_web_service
                                    $servicio->tipo, // $tipo
                                    $servicio->rol, // $rol
                                    $pdi->certificado_ssl, //$certificado
                                    'OK', //$error_texto
                                    false, //$error
                                    $service_soap, //$soap_peticion
                                    $soap_response, //$soap_respuesta
                                    $servicio->id, //$catalogo_id
                                    false, //$seguridad - seguridad no se usa por ser pdi,
                                    $etapa,
                                    $fecha_ejecucion_servicio
                                  );
              }
    ...
    private function procesar_errores_pdi($etapa, $curl_errno,$curl_error,$http_code, $body,$soap_response,$xml,$servicio,$pdi,$fecha_ejecucion_servicio){
      ...
      if($curl_errno > 0) {
        ...
        $this->guardar_monitoreo(
                                  $pdi->sts, //$url_web_service
                                  $servicio->tipo, //$tipo
                                  $servicio->rol, //$rol
                                  $pdi->certificado_ssl, //$certificado
                                  $curl_error.$http_code_texto, //$error_texto
                                  true, //$error
                                  $body, //$soap_peticion
                                  $soap_response, //$soap_respuesta
                                  $servicio->id, //$catalogo_id
    ...
    private function procesar_errores_soap($etapa, $curl_errno,$curl_error,$http_code, $soap_body,$soap_response,$xml,$soap_endpoint_location,$servicio,$fecha_ejecucion_servicio){
      ...
      if ($hubo_error){
        ...
        $this->guardar_monitoreo(
                                  ...
                                  '', //$rol - rol no se usa por ser soap
                                  '', //$certificado - certificado no se usa por ser soap
                                  $dato->valor.$http_code_texto, //$error_texto
                                  true, //$error
                                  $soap_body, //$soap_peticion
                                  $soap_response, //$soap_respuesta
                                  $servicio->id, //$catalogo_id
    ...
    private function guardar_monitoreo($url_web_service, $tipo, $rol, $certificado, $error_texto, $error, $soap_peticion, $soap_respuesta, $catalogo_id, $seguridad, $etapa, $fecha_ejecucion_servicio){

      $paso = $this->obtener_paso_actual($etapa);

      $monitoreo = new Monitoreo();
      $monitoreo->url_web_service = $url_web_service;
      $monitoreo->fecha = $fecha_ejecucion_servicio;
      $monitoreo->tipo = $tipo;
      $monitoreo->rol = $rol;
      ...
      $monitoreo->proceso_id = $etapa->Tramite->Proceso->id;
      $monitoreo->tramite_id = $etapa->Tramite->id;
      $monitoreo->etapa_id = $etapa->id;
      $monitoreo->paso_id = $paso->id;
      $monitoreo->fecha_respuesta_servicio = date('Y-m-d', time()).' '.date('H:i:s', time());
      $monitoreo->save();
    }
    ...
    private function obtener_paso_actual($etapa){
      $ejecutar_fin = false;
