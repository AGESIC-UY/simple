...
class Pasarela_pagos extends MY_BackendController {
    ...
    public function crear() {
        ...
        $pasarela->save();
        //auditar('PasarelaPago', "insert", $pasarela->id, UsuarioBackendSesion::usuario()->usuario);
    ...
    public function eliminar($pasarela_id) {
        ...
        auditar('PasarelaPago', "delete", $pasarela->id, UsuarioBackendSesion::usuario()->usuario);
        $pasarela->delete();
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            switch ($this->input->post('metodo')) {
                ...
                case 'antel':
                    ...
                    $pasarela_metodo->descripcion_pendiente_traza = $this->input->post('pasarela_metodo_antel_descripcion_pendiente_traza');
                    $pasarela_metodo->descripcion_iniciado_traza = $this->input->post('pasarela_metodo_antel_descripcion_iniciado_traza');
                    $pasarela_metodo->descripcion_token_solicita_traza = $this->input->post('pasarela_metodo_antel_descripcion_token_solicita_traza');
                    $pasarela_metodo->descripcion_realizado_traza = $this->input->post('pasarela_metodo_antel_descripcion_realizado_traza');
                    $pasarela_metodo->descripcion_error_traza = $this->input->post('pasarela_metodo_antel_descripcion_error_traza');
                    $pasarela_metodo->descripcion_reachazado_traza = $this->input->post('pasarela_metodo_antel_descripcion_reachazado_traza');
                    $pasarela_metodo->referencia_pago = $this->input->post('pasarela_metodo_antel_referencia_pago');
                ...
                case 'generico':
                    ...
                    if (!$this->input->post('pasarela_metodo_generico_descripciones_estados_traza')) {
                        $pasarela_metodo->descripciones_estados_traza = null;
                    } else {
                        $descripciones_estados_traza = $this->input->post('pasarela_metodo_generico_descripciones_estados_traza');
                        $count = 1;

                        foreach ($descripciones_estados_traza as $dato) {
                            if (trim($dato['codigo']) == '' && trim($dato['valor']) == '') {
                                unset($descripciones_estados_traza[$count]);
                            }
                            $count++;
                        }

                        if (count($descripciones_estados_traza) > 0) {
                            $pasarela_metodo->descripciones_estados_traza = json_encode($descripciones_estados_traza);
                        } else {
                            $pasarela_metodo->descripciones_estados_traza = null;
                        }
                    }
            ...
            auditar('PasarelaPago', $op, $pasarela->id, UsuarioBackendSesion::usuario()->usuario);
