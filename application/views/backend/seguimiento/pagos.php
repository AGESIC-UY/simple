...
<div id='busquedaAvanzada' style='display: <?= $busqueda_avanzada ? 'block' : 'none' ?>;'>
    <form class='form-horizontal' method="get" action="<?= current_url() ?>">
        <fieldset>
            ...
            <div class='row-fluid'>
                <div class='span6'>
                    <div class='control-group' id="id_solicitud" >
                        <label class='control-label' for="idTramiteInterno">ID trámite interno</label>
                        <div class='controls'>
                            <input type='text' name='idTramiteInterno' id="idTramiteInterno"   value='<?= $idTramiteInterno ?>'/>                           
                        </div>
                    </div>
                </div>
                <div class='span6'>
                    <div class='control-group' id="id_solicitud" >
                        <label class='control-label' for="idSolicitud">ID solicitud</label>
                        <div class='controls'>
                            <input type='text' name='idSolicitud' id="idSolicitud"  value='<?= $idSolicitud ?>'/>                           
                        </div>
                    </div>
                </div>
            </div>
    ...
    <tbody>
        <?php foreach ($registros as $p): ?>
            <tr>
                ...
                <td><?= $p->pasarela ?></td>
                <td><span class="estado badge <?= ($p->estado == 'realizado' || $p->estado == '25' ? 'badge-success' : ($p->estado == 'error' ? 'badge-important' : ($p->estado == 'pendiente' ? 'badge-warning' : ($p->estado == 'rechazado' ? 'badge-important' : 'badge-secondary')))) ?>"><?= $p->estado ?></span></td>
