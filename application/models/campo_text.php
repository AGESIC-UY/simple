...
class CampoText extends Campo {
    ...
    protected function display($modo, $dato, $etapa_id) {
        ...
        if ($this->documento_tramite) {
            $display .= '<input class="hidden" title="Numero de documento del trámite" type="text" name="documento_tramite_inicial__e' . $etapa_id . '" value="' . $this->nombre . '" />';
        }

        if ($this->email_tramite) {
            $display .= '<input class="hidden" title="Correo electrónico del trámite" type="text" name="email_tramite_inicial__e' . $etapa_id . '" value="' . $this->nombre . '" />';
        }
